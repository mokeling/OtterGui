...
public class ItemSelector<T>
{
    ...
    private void DrawFilter(float width)
    {
        ...
        var enterPressed = ImGui.InputTextWithHint(string.Empty, "ɸѡ...", ref newFilter, 64, ImGuiInputTextFlags.EnterReturnsTrue);
        if (newFilter != Filter)
        {
            Filter      = newFilter;
            FilterDirty = true;
